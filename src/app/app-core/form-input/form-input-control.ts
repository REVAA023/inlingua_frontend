...
export class FromInputControl extends FormAccessorBase {
  ...
  __helpText = '';
  @Input() set helpText(helpText: string) {
  ...
  @Input() set designType(designType: string) {
    this.__designType = designType || '';
  }
  get designType() {
    return this.__designType;
  }

  __tabIndex = 0;
  @Input() set tabIndex(tabIndex: number) {
    this.__tabIndex = tabIndex || 0;
  }
  get tabIndex() {
  ...
  __upperCase = false;
  @Input() set upperCase(upperCase: boolean) {
    this.__upperCase = upperCase || false;
  }
  get upperCase() {
  ...
  __lowerCase = false;
  @Input() set lowerCase(lowerCase: boolean) {
    this.__lowerCase = lowerCase || false;
  }
  get lowerCase() {
    return this.__lowerCase;
  }

  __errorTrue = false;
  @Input() set errorTrue(errorTrue: boolean) {
    this.__errorTrue = errorTrue || false;
  }
  get errorTrue() {
  ...
  @Input() set btnClicked(btnClicked: boolean) {
    this.__btnClicked = btnClicked || false;
  }
  get btnClicked() {
    return this.__btnClicked;
  }

  __errorText = 'Mandatory';
  @Input() set errorText(errorText: string) {
    this.__errorText = errorText || 'Mandatory';
  }
  get errorText() {
  ...
  __userIcon = false;
  @Input() set userIcon(userIcon: boolean) {
    this.__userIcon = userIcon;
  }
  get userIcon() {
    return this.__userIcon;
  }

  __passwordStrength = false;
  @Input() set passwordStrength(passwordStrength: boolean) {
    this.__passwordStrength = passwordStrength || false;
  }
  get passwordStrength() {
    return this.__passwordStrength;
  }

  __noEmoji = false;
  @Input() set noEmoji(noEmoji: boolean) {
    this.__noEmoji = noEmoji;
  }
  get noEmoji() {
    return this.__noEmoji;
  }

  // Input Validation Setting
  __decimal = 0;
  @Input() set decimal(decimal: number) {
    this.__decimal = decimal || 0;
  ...
  @Input() set required(required: boolean) {
    ...
    this.__required = required || false;
    this.setValidate(this.xControl);
  }
  get required() {
  ...
  __valueType = '';
  @Input() set valueType(valueType: string) {
  ...
  __keyName = '';
  @Input() set keyName(keyName: string) {
  ...
  @Input() set multipleSelect(multipleSelect: boolean) {
    this.__multipleSelect = multipleSelect;
  }
  get multipleSelect() {
    return this.__multipleSelect;
  }

  __enableSelectSearch = false;
  @Input() set enableSelectSearch(enableSelectSearch: boolean) {
    this.__enableSelectSearch = enableSelectSearch;
  }
  get enableSelectSearch() {
    return this.__enableSelectSearch;
  }

  // Other Requirements
  __oldValue: any = null;
  @Input() set oldValue(oldValue: any) {
    this.__oldValue = oldValue || null;
  }
  get oldValue() {
  ...
  @Input()
  set oldValueDescription(oldValueDescription: any) {
    this.__oldValueDescription = oldValueDescription || null;
  }
  ...
  strengthParams: { key: string; regex: RegExp; main: string; small: string; isValid: boolean }[] = []; // Add this property
  ...
  getPasswordStrength() {
    ...
    const strengthCriteria = [
      { key: 'eight_characters', regex: /.{8,}/, main: '8+', small: 'Characters' },
      { key: 'lower_case', regex: /[a-z]/, main: 'Aa', small: 'Lowercase' },
      { key: 'upper_case', regex: /[A-Z]/, main: 'AA', small: 'Uppercase' },
      { key: 'number', regex: /\d/, main: '123', small: 'Numbers' },
      { key: 'special_characters', regex: /[!@#$%^&*()_\-+={}[\]|:;"'<>,.?/~]/, main: '&#64;$', small: 'Special Characters' },
    ];
  
    this.__strengthParams = {};
    let strengthNumber = 0;
  
    this.strengthParams = strengthCriteria.map(param => {
      const isValid = param.regex.test(password);
      if (isValid) strengthNumber++;
      return { ...param, isValid };
    });
  
    this.__strength = (strengthNumber / strengthCriteria.length) * 100;
  }
  ...
  toggleHide() {
    ...
    this.__passwordTextType === 'password'
      ? (this.__passwordTextType = 'text')
      : (this.__passwordTextType = 'password');
  ...
  // Newly add Condition
  __inputIcon = '';
  __inputIconIsSrc = false;
  __isInputIcon = false;
  __contentEditable = false;
  __dropdownOpen = false;
  __selectedCountry = country;
  ...
  @Input()
  set contentEditable(contentEditable: boolean) {
    this.__contentEditable = contentEditable || false;
  }
  get contentEditable() {
    return this.__contentEditable;
  }
  
  @Input()
  set inputIconIsSrc(inputIconIsSrc: boolean) {
    this.__inputIconIsSrc  = inputIconIsSrc || false;
  }
  get inputIconIsSrc() {
    return this.__inputIconIsSrc ;
  }
  
  @Input()
  set isInputIcon(isInputIcon: boolean) {
    this.__isInputIcon  = isInputIcon || false;
  }
  get isInputIcon() {
    return this.__isInputIcon ;
  }
  
  @Input()
  set inputIcon(inputIcon: string) {
    this.__inputIcon = inputIcon || '';
  }
  get inputIcon() {
    return this.__inputIcon;
  }

  toggleDropdown() {
    this.__dropdownOpen = !this.__dropdownOpen;
  }

  selectCountry(country: any) {
    this.__selectedCountry = country;
